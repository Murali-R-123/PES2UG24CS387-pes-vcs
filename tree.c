...
// Builds tree for a given prefix (directory level)
static int build_tree(Index *index, const char *prefix, ObjectID *out_oid) {
    Tree tree;
    tree.count = 0;
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        // Check if path belongs to this directory level
        if (strncmp(path, prefix, prefix_len) != 0)
            continue;

        const char *rest = path + prefix_len;

        // Skip deeper levels not belonging directly
        if (strchr(rest, '/') == NULL) {
            // FILE at this level
            ...
            ObjectID child_oid;
            if (build_tree(index, new_prefix, &child_oid) != 0)
                return -1;
    ...
    if (object_write(OBJ_TREE, data, len, out_oid) != 0) {
        free(data);
        return -1;
    }
